var SpellingBuddy = (() => {
  ...
  __export(index_exports, {
    ...
    lerp: () => lerp,
    lettersToVisemes: () => lettersToVisemes,
  ...
  function faceFrame(S) {
    ...
    const vis = smooth(-0.02, 0.3, hole.z / G.Rf);
    const eye = (p, dx, dy) => ({
      x: p.x + dx,
      y: p.y + dy,
      fx: Math.abs(p.fx),
  ...
  var GLYPHS = {
    ...
    G: [
      arcPath(0, 0.02, 0.32, 0.48, 8, 300),
    ...
    M: [sub(["M", -0.34, B], ["L", -0.34, T], ["L", 0, 0.1], ["L", 0.34, T], ["L", 0.34, B])],
    ...
    Q: [
      ...arcPathWrap(0, 0, 0.32, 0.5, 0, 360),
  ...
  var LOWER = {
    a: [
      arcPath(-0.02, XC, 0.25, XR, 0, 360),
      sub(["M", 0.23, X], ["L", 0.23, B])
    ],
    b: [
      sub(["M", -0.25, T], ["L", -0.25, B]),
      arcPath(0, XC, 0.25, XR, 0, 360)
    ],
    ...
    f: [
      [
        ["M", -0.09, B],
        ["L", -0.09, -0.3],
        ["C", -0.09, -0.47, 0.02, -0.5, 0.15, -0.46]
      ],
  ...
  var DIGITS = {
    ...
    5: [
      sub(["M", 0.22, T], ["L", -0.22, T], ["L", -0.24, -0.02]),
      arcPath(0, 0.2, 0.26, 0.3, -100, 150)
    ],
    6: [
      [...arcPath(0, 0.18, 0.26, 0.32, 0, 360)],
      sub(["M", -0.26, 0.18], ["C", -0.26, -0.28, -0.06, T, 0.2, -0.44])
    ],
    7: [sub(["M", -0.26, T], ["L", 0.26, T], ["L", -0.06, B])],
    8: [
      arcPath(0, -0.24, 0.22, 0.26, 0, 360),
  ...
  function scoreTrace(ch, input, { tolerance = 0.16, diagnose = false, candidates } = {}) {
    ...
    const paths = (nested ? input : [input]).map((path) => resample(path.map(pt), tolerance * 0.5)).filter((path) => path.length >= 2);
    ...
    const mirrored = (Array.isArray(input[0]) && Array.isArray(input[0][0]) ? input : [input]).map((path) => path.map((p) => Array.isArray(p) ? [-p[0], p[1]] : [-p.x, p.y]));
  ...
  function drawHeldLetter(s, S, T2) {
    ...
    s.fill(T2.face);
    s.stroke(T2.body, 4);
    drawGlyph(s, S.heldLetter, 30, T2.body, 0.145, true, "ink");
    s.restore();
  }
